...
def init():
    ...
    for row_index in range(1, table.nrows):
        ...
        for i in range(len(header_row)):
            ...
            # 把数字转换为Long类型,存在风险,必须要事先明确Excel表格当中的数字不是浮点数
            value = cell.value
        ...
        xm = data_row0[key_xm_index]
        zjhm = data_row0[key_zjhm_index]
        ksbh = data_row0[key_ksbh_index]

        key = u'%s#%s#%s' % (xm.value, zjhm.value, ksbh.value)
